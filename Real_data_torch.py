...
##-----------------------------------------
del f_est, Sqs_est, diff
gc.collect()
torch.cuda.empty_cache()
##-----------------------------------------

###########################################
# Apply admm. Attention, here I use R_0!
###########################################

t0 = time.perf_counter()

lambdas, f_hat, history = dast.admm(   	qs_norm, thetas, weights, Sqs, sigma = sigma,
										R_csr = 0 * R25, f0 = None, normalize = True,
										Lambdas = None, rho = 0.5, rho_ratio = 2, dynamic_rho = True,
										beta = 0.5, c = 1e-4, tol = 1e-6, epsilon = 1e-8, maxiter = 10,
										cg_rtol = 1e-10, cg_atol = 1e-50, cg_maxiter = 1000,
...
# torch.save(state, "/work/xunan/MaxEnt/Results/R_0_solution.pt")
# del state
...
alpha_list = torch.tensor([0.1, 0.25, 0.5, 1, 1.5, 2]) # 
beta_list = torch.tensor([0]) # , 1e0, 1e1, 1e2, 1e3
...
torch.save(state, "/work/xunan/MaxEnt/Results/CV_results.pt")
