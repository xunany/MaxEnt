...
def admm(   qs, thetas, weights, Sqs, sigma, R_csr, f0 = None, normalize = True, Lambdas = None, 
    ...
    def dist_Newton_Armijo( Lambdas, zs, us, rho, ker_mat, ker_prod, weights, f0, normalize,
        ...
        # cholesky solver, may don't work when the hess is not numerically psd
        ...
        # general solver, which usually doesn't work well when cholesky colver doesn't work too
        # direction = -np.linalg.solve(hess, grad[..., None]).squeeze(-1)                     # in shape (V, M)

        # cholesky + adaptive jitter. It gradually adds nugget to the hess matrix
        # I = np.eye(M)[None,:,:]
        # eps = 1e-16
        # on_off = 0
        # while True:
        #     try:
        #         L = np.linalg.cholesky(hess + on_off*eps*I)                                        # in shape (V, M, M)
        #         break
        ...
        return Lambdas + direction                                                          # in shape (V, M)
    ...
    while True:
        ...
        Lambdas = dist_Newton_Armijo(   Lambdas, zs.reshape(V, M), us.reshape(V, M), rho, ker_mat, ker_prod, 
