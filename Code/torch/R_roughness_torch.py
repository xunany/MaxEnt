...
def R_all(v, m, order=1):
    ...
    # Convert scipy csr to torch.sparse_csr_tensor
    indptr = torch.tensor(R.indptr)
    indices = torch.tensor(R.indices)
    values = torch.tensor(R.data)
    shape = R.shape

    R_torch = torch.sparse_csr_tensor(indptr, indices, values, size=shape)
